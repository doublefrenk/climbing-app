...
import { selectAllRoutes } from "../store/routesSlice.js"
...
export default function About() {
  ...
  const handleFileChange = async (e) => {
    ...
    for (const file of files) {
      ...
      try {
        const response = await authFetch("/api/users/upload", { method: "POST", body: formData }, getToken)
        const data = await response.json()
        if (response.ok) {
          setGallery((prev) => [...prev, { id: Date.now(), title: data.title, image: data.url }])
        } else {
            ...
            <table className="w-full bg-white shadow-sm border border-gray-200 rounded-lg ml-10 mt-4">
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {getHardestRoute(routes[selectedCategory]).map((route) => (
                  <MyClimbingRoute key={route.id} route={route} handleDeleteRoute={null} handleEditRoute={null} />
                ))}
              </tbody>
            </table>
          </div>
